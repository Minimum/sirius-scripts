...
smartctl -H "/dev/nvme0n1" >> $l_log_path

echo "/dev/sda:" >> $l_log_path
smartctl -H "/dev/sda" >> $l_log_path

echo "/dev/sdb:" >> $l_log_path
smartctl -H "/dev/sdb" >> $l_log_path

echo "/dev/sdc:" >> $l_log_path
smartctl -H "/dev/sdc" >> $l_log_path

echo "/dev/sdd:" >> $l_log_path
smartctl -H "/dev/sdd" >> $l_log_path

echo "/dev/sde:" >> $l_log_path
smartctl -H "/dev/sde" >> $l_log_path

echo "/dev/sdf:" >> $l_log_path
smartctl -H "/dev/sdf" >> $l_log_path

echo "/dev/sdg:" >> $l_log_path
smartctl -H "/dev/sdg" >> $l_log_path

echo "/dev/sdh:" >> $l_log_path
smartctl -H "/dev/sdh" >> $l_log_path

# Close email
echo "</pre></body></html>" >> $l_log_path

# Send email
msmtp "$l_email" < $l_log_path
